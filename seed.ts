...
const KC = [0x9e3779b9, 0x3c6ef373, 0x78dde6e6, 0xf1bbcdcc, 0xe3779b99, 0xc6ef3733, 0x8dde6e67, 0x1bbcdccf, 0x3779b99e, 0x6ef3733c, 0xdde6e678, 0xbbcdccf1, 0x779b99e3, 0xef3733c6, 0xde6e678d, 0xbcdccf1b];

class Seed {
    public static GetB0(A: number) {
        return (0x000000ff & (A));
    }

    public static GetB1(A: number) {
        return (0x000000ff & ((A) >>> 8));
    }

    public static GetB2(A: number) {
        return (0x000000ff & ((A) >>> 16));
    }

    public static GetB3(A: number) {
        return (0x000000ff & ((A) >>> 24));
    }

    public EndianChange(a: number[]) {
        a[0] = (a[0] >>> 24) | (a[0] << 24) | ((a[0] << 8) & 0x00ff0000) | ((a[0] >>> 8) & 0x0000ff00);
    }

    public _EndianChange(a: number) {
        return (a >>> 24) | (a << 24) | ((a << 8) & 0x00ff0000) | ((a >>> 8) & 0x0000ff00);
    }

    public static SeedRound(a: number[], b: number[], c: number[], d: number[], K: number[]) {
        var e, T1;
        var f = 0, T11 = 0;
        e = c[0] ^ K[0];
        T1 = d[0] ^ K[1];
        ...
        T1 = SS0[Seed.GetB0(T1)] ^ SS1[Seed.GetB1(T1)] ^ SS2[Seed.GetB2(T1)] ^ SS3[Seed.GetB3(T1)];
        T11 = (T1 < 0) ? (T1 & 0x7fffffff) | (0x80000000) : (T1);
        f += T11;
        e = SS0[Seed.GetB0(f)] ^ SS1[Seed.GetB1(f)] ^ SS2[Seed.GetB2(f)] ^ SS3[Seed.GetB3(f)];
        f = (e < 0) ? (e & 0x7fffffff) | (0x80000000) : (e);
        T11 += f;
        T1 = SS0[Seed.GetB0(T11)] ^ SS1[Seed.GetB1(T11)] ^ SS2[Seed.GetB2(T11)] ^ SS3[Seed.GetB3(T11)];
        T11 = (T1 < 0) ? (T1 & 0x7fffffff) | (0x80000000) : (T1);
        f += T11;
        a[0] ^= f;
        b[0] ^= T11;
    ...
    public static SeedEncrypt(a: number[], b: number[], c: number[]) {
        var d = new Array(1);
        var e = new Array(1);
        var f = new Array(1);
        var g = new Array(1);
    ...
    public static SeedDecrypt(a: number[], b: number[], c: number[]) {
        var d = new Array(1);
        var e = new Array(1);
        var f = new Array(1);
        var g = new Array(1);
    ...
    public static EncRoundKeyUpdate0(K: number[], A: number[], B: number[], C: number[], D: number[], Z: number) {
        var a, b, T11;
        a = A[0];
        A[0] = (A[0] >>> 8) ^ (B[0] << 24);
        B[0] = (B[0] >>> 8) ^ (a << 24);
        b = A[0] + C[0] - KC[Z];
        T11 = B[0] + KC[Z] - D[0];
        K[0] = SS0[Seed.GetB0(b)] ^ SS1[Seed.GetB1(b)] ^ SS2[Seed.GetB2(b)] ^ SS3[Seed.GetB3(b)];
        K[1] = SS0[Seed.GetB0(T11)] ^ SS1[Seed.GetB1(T11)] ^ SS2[Seed.GetB2(T11)] ^ SS3[Seed.GetB3(T11)];
    }

    public static EncRoundKeyUpdate1(K: number[], A: number[], B: number[], C: number[], D: number[], Z: number) {
        var a, b, T11;
        a = C[0];
        C[0] = (C[0] << 8) ^ (D[0] >>> 24);
        D[0] = (D[0] << 8) ^ (a >>> 24);
    ...
    public static SeedRoundKey(a: number[], b: number[]) {
        ...
        c = A[0] + C[0] - KC[0];
        T1 = B[0] - D[0] + KC[0];
        a[0] = SS0[Seed.GetB0(c)] ^ SS1[Seed.GetB1(c)] ^ SS2[Seed.GetB2(c)] ^ SS3[Seed.GetB3(c)];
        a[1] = SS0[Seed.GetB0(T1)] ^ SS1[Seed.GetB1(T1)] ^ SS2[Seed.GetB2(T1)] ^ SS3[Seed.GetB3(T1)];
    ...
    public static SeedXor(a: number[], b: number[], c: number[]) {
        var i = 0;
    ...
    public static ArrayCopy(a: number[], b: number, c: number[], d: number, e: number) {
        for (var i = b; i < (b + e); i++) c[d++] = a[i];
    }

    public static SeedEncryptEcb(a: number[], b: number[], c: number[]) {
        Seed.SeedEncrypt(b, c, a);
    }

    public static SeedDecryptEcb(a: number[], b: number[], c: number[]) {
        Seed.SeedDecrypt(b, c, a);
    }

    public static SeedEncryptCbc(a: number[], b: number[], c: number[], d: number, e: number[]) {
        ...
        if (remainLen != 0) Seed.ArrayCopy(c, i * 16, f, i * 16, remainLen);
    ...
    public static SeedEnc(geo: string, sessionKey: number[]) {
		...
		for (var i = 0; i < geo.length; i++) {
			...
			inData[i] = Number(geo.charAt(i).toString());
		}
		...
		for (var i = 0; i < 16; i++) encodedDataString += Number(outData[i]).toString(16) + ",";
